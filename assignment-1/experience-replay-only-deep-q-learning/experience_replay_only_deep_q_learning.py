...
class QNetwork(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.fc = nn.Sequential(
            nn.Linear(input_dim, 24),  # Small hidden layer
            nn.ReLU(),
...
class DQNWithExperienceReplay:
    ...
    def store_experience(self, state, action, reward, next_state, done):
        """Stores transition in experience replay buffer."""
...
# Training function
def train_dqn_with_experience_replay():
    all_results = []

    for run in range(NUM_RUNS):
        agent = DQNWithExperienceReplay(env)
        run_results = []
...
# Run multiple iterations of DQN with Experience Replay
results = train_dqn_with_experience_replay()
...
csv_path = os.path.join(output_dir, "dqn_with_experience_replay_results.csv")
plot_path = os.path.join(output_dir, "dqn_with_experience_replay_plot.png")
...
plt.title("CartPole Performance - DQN with Experience Replay")
plt.legend()
plt.grid()
