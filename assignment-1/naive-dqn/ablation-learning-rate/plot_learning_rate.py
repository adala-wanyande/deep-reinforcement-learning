...
# Load CSV file
csv_path = "../data/ablation_learning_rate_results.csv"
df = pd.read_csv(csv_path)

# Ensure data is grouped by learning rate
configurations = df["Learning Rate"].unique()
...
SMOOTHING_WINDOW = 10000  # Adjusted to provide a balanced smoothness

# Prepare the figure
plt.figure(figsize=(10, 5))

for config in configurations:
    df_config = df[df["Learning Rate"] == config]
    ...
    plt.plot(steps, smoothed_means, label=f"LR = {config}")
    plt.fill_between(
        steps,
...
plt.title("Ablation Study on Learning Rate (Steps vs Reward)", fontsize=14)

# Improve the legend
plt.legend(title="Learning Rate", fontsize=10, title_fontsize=12, loc="upper left", frameon=True)
...
plot_path = "../visuals/ablation_learning_rate_plot.png"
