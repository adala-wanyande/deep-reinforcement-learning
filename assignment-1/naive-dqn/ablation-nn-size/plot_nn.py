...
csv_path = "../data/ablation_network_size_results.csv"
df = pd.read_csv(csv_path)

# Ensure data is grouped by network size
configurations = df["Network Size"].unique()
...
for config in configurations:
    df_config = df[df["Network Size"] == config]
...
plt.title("Ablation Study on Neural Network Size (Steps vs Reward)")
plt.legend(title="Network Size", fontsize=10, title_fontsize=12, loc="upper left", frameon=True)
...
plot_path = "../visuals/ablation_network_size_plot.png"
