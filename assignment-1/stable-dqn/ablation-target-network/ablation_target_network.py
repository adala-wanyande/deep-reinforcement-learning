...
# DQN Agent with Target Network (No Experience Replay)
class DQNAgent:
    ...
    def train(self, state, action, reward, next_state, done):
        ...
        next_state = torch.tensor(next_state, dtype=torch.float32).unsqueeze(0)  # Ensure batch dimension
        action = torch.tensor([action], dtype=torch.int64).unsqueeze(1)  # Ensure batch dimension
        ...
        with torch.no_grad():
            next_q_values = self.target_network(next_state).max(1)[0]  # Get max Q-value for next state
            target_q_values = reward + GAMMA * next_q_values * (1 - done)

        # Compute loss and update
        loss = self.criterion(q_values, target_q_values)  # No need to squeeze
...
def train_dqn(run_id):
    ...
    while total_steps < MAX_ENV_STEPS:
        ...
        while not done:
            ...
            agent.train(state, action, reward, next_state, done)  # Train on the most recent experience
