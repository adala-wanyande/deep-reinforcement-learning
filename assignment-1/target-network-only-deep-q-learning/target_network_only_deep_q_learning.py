...
class DQNWithTargetNetwork:
    def __init__(self, env):
        ...
        self.target_network.load_state_dict(self.q_network.state_dict())  # Sync target network initially
        self.target_network.eval()  # Target network does not update during training
    ...
    def update_target_network(self):
        """Update target network weights to match the Q-network."""
...
# Training function
def train_dqn_with_target_network():
    all_results = []

    for run in range(NUM_RUNS):
        agent = DQNWithTargetNetwork(env)
        run_results = []
...
# Run multiple iterations of DQN with target network
results = train_dqn_with_target_network()
...
csv_path = os.path.join(output_dir, "dqn_with_target_network_results.csv")
plot_path = os.path.join(output_dir, "dqn_with_target_network_plot.png")
...
plt.title("CartPole Performance - DQN with Target Network")
plt.legend()
plt.grid()
